...
private:
    ...
    Eigen::VectorXd forward_kinematics()
    {
        ...
        Eigen::VectorXd x(2);
        x << l1_*cos(q1)+l2_*cos(q1+q2), l2_*sin(q1)+l2_*sin(q1+q2);
    ...
    Eigen::VectorXd impedance_controller()
    {
        ...
        x_error << forward_kinematics() - equilibrium_pose_;
        Eigen::VectorXd x_dot_error(2);
        x_dot_error << differential_kinematics();

        // Replace with actual impedance controller equation: x'' = M^(-1)[F_ext - k x_error - B x'_error]
        Eigen::VectorXd x_ddot(2);
        x_ddot << mass_matrix_.inverse()*(-damping_matrix_*x_dot_error - damping_matrix_*x_error + external_wrenches_);
    ...
    Eigen::VectorXd calculate_desired_joint_accelerations()
    {
        ...
        RCLCPP_INFO(this->get_logger(), "x_ddot: [%.3f, %.3f]", desired_cartesian_accelerations_(0), desired_cartesian_accelerations_(1));

        Eigen::VectorXd q_ddot(2);

        q_ddot << jacobian_.inverse()*(impedance_controller()-jacobian_derivative_*joint_velocities_);
